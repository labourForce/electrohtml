...
public class ShoppingCartMapper {
    ...
    public static ShoppingCartDTO createShoppingCartDTO(ShoppingCart cart) {

        ShoppingCartDTO shoppingCartDTO = new ShoppingCartDTO();
        OrderDTO orderDTO = new OrderDTO();
        List<ProductDTO> productList = new ArrayList<>();
        if (cart!=null) {
            if (cart.getCartProperties() != null) {
                double orderTotalPrice = 0;
                int orderTotalQuantity = 0;
                for (ShoppingCartProperties property : cart.getCartProperties()) {
                    orderTotalQuantity++;
                    CommerceItem commerceItem = property.getItem();
...
//                      variableSkuDTO.setFeaturedImage(variableSKU.getImage());
                ...
                orderDTO.setOrderUID(cart.getId());
